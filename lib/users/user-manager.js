...
(() => {
  ...
  class UserManager extends EventEmitter {
    ...
    create({username, password, scopes=null, isAnonymous=false, salt=null, hash=null}={}) {
      let inSalt = salt;
      let inHash = hash;
      if (!this._isValidUsername(username)) {
        ...
        .then((userId) => {
          const salt = inSalt || this._createPasswordSalt();
          const passwordHash = inHash || this._createPasswordHash(password, salt);
          const now = Date.now();
