...
(() => {
  ...
  class UserManager extends EventEmitter {
    constructor(messageCenter, scopeManager) {
      ...
      this._flagDbPath = path.join(this._storePath, '/flags');
    ...
    load(server) {
      ...
      .then((val) => {
        const scopeList = SCOPE_FLAGS.filter((scope) => !val.hasOwnProperty(scope));
        if (scopeList.length === 0) {
          return Promise.resolve();
        }
        return Promise.resolve()
        .then(() => this.list({}))
        .then((users) => Promise.all(users.map((user) => this._giveUserScopes(user, scopeList))))
        .then(() => scopeList.reduce((obj, scope) => Object.assign(obj, {[scope]: 1}), val))
        .then((newVal) => this._flagDb.put('/flag', newVal));
      });
    }
    ...
    _giveUserScopes(user, scopes) {
      return Promise.resolve()
      .then(() => {
        const allStaticScopes = ScopeManager.ALL_STATIC_SCOPES;
        if (!Array.isArray(user.scopes)) {
          user.scopes = [];
        }
        let scopeLength = user.scopes.length;
        ...
        if (user.scopes.length !== scopeLength) {
