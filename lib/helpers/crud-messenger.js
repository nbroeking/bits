...
(() => {
  ...
  class CrudMessenger extends Messenger {
    constructor(tag, manager, {
      ...
      this.addRequestListener(`${this._tag} create`, this._writeScopes, this._create.bind(this));
      this.addRequestListener(`${this._tag} count`, this._readScopes, this._count.bind(this));
      this.addRequestListener(`${this._tag} list`, this._readScopes, this._list.bind(this));
      this.addRequestListener(`${this._tag} get`, this._readScopes, this._get.bind(this));
      this.addRequestListener(`${this._tag} update`, this._writeScopes, this._update.bind(this));
      this.addRequestListener(`${this._tag} delete`, this._writeScopes, this._delete.bind(this));
    ...
    sanitize(item) {
      /**
       * For now, this just returns what is passed in. If this functionality
       * changes in the future, we will need to handle the case where this is
       * an array or single object
       */
    ...
    _list(metadata, query, options) {
      ...
      .then((items) => Promise.all(items.map((item) => this.sanitize(item))));
    }
