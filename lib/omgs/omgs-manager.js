...
(() => {
  ...
  class OmgsManager extends EventEmitter {
    ...
    load(baseServer, messageCenter) {
      ...
      .then(() => this._moduleManager.on('upgrade-starting', this._boundOnUpgradeStarting));
    }

    unload(base) {
      return Promise.resolve()
      .then(() => this._moduleManager.removeListener('upgrade-starting', this._boundOnUpgradeStarting))
      .then(() => {
        this._omgsDir = null;
        this._rawsDir = null;
        this._infosDir = null;
