...
(() => {
  ...
  const LOAD_PROCESS_FINISH_EVENT = 'base-module-finished-load';
  ...
  class ModuleManager extends EventEmitter {
    ...
    filterModule(module) {
      ...
      return {
        ...
        installedDir: module.installedDir
      };
    }
    ...
    loadModules(mods, {ignoreFailures = false} = {}) {
      return Promise.resolve()
      .then(() => this.setManagerState('Loading'))
    ...
    loadModule(module) {
      ...
      if (mod.isBase) {
        ...
        .then(() => this.emit('module list change', this._modules))
        .then(() => mod)
        .catch((err) => {
          logger.error('Unable to load base', err);
      ...
      .then(() => {
        return new Promise((resolve, reject) => {
          ...
          const loadCallFinished = (err, result) => {
            ...
            this._messageCenter.removeEventListener(LOAD_PROCESS_FINISH_EVENT, loadCallFinished);
            if (err) {
              reject(err);
            } else {
              resolve(result);
          ...
          this._messageCenter.addEventListener(LOAD_PROCESS_FINISH_EVENT, null, loadCallFinished);
