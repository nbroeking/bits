...
(() => {
  ...
  class ModuleRouter extends CrudRouter {
    constructor(manager, {readScopes=null, writeScopes=null, routePath}) {
      ...
      this._router.post('/module-package', upload.any(), (...data) => this._upload(...data));
    }
    ...
    _upload(req, res, next) {
      if (!Array.isArray(req.files) || 0 >= req.files.length) {
        ...
        Promise.all(files.map((file) => {
          const modPath = path.resolve(this._modulesPackagesUploadDir, file.filename + '-mod');
          const command = 'tar';
          const args = [
            '--warning=none',
            '-x',
          ...
          .then(() => {
            logger.debug('Attempting to untar module');
            return UtilChildProcess.spawn(command, args);
          })
          .then((result) => {
            logger.debug('Tar completed ------');
            if (result.code !== 0) {
          ...
          .then(() => {
            ...
            .then((info) => {
              if (!info) {
                ...
                return this._moduleManager.installModule(modPath);
              }
            });
          });
        }))
